        ...
        <div class="content">
            <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form">
                <table class="list">
                    ...
                    <?php if ($remont) { ?>
                    <?php $class = 'odd'; ?>
                    <?php foreach ($remont as $remont_story) { ?>
                    <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                    <tr class="<?php echo $class; ?>">
                        <td align="center"><?php if ($remont_story['selected']) { ?>
                            <input type="checkbox" name="selected[]" value="<?php echo $remont_story['remont_id']; ?>" checked="checked" />
                            <?php } else { ?>
                            <input type="checkbox" name="selected[]" value="<?php echo $remont_story['remont_id']; ?>" />
                            <?php } ?></td>
                        <td class="left"><?php echo $remont_story['title']; ?></td>
                        <td class="left"><?php echo $remont_story['date_added']; ?></td>
                        <td class="right"><?php echo $remont_story['status']; ?></td>
                        <td class="right"><?php foreach ($remont_story['action'] as $action) { ?>
