...
class ControllerPaymentCardTransfer extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('card_transfer', $this->request->post);
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/card_transfer', 'token=' . $this->session->data['token'], 'SSL'),
            'separator' => ' :: '
        );

        $this->data['action'] = $this->url->link('payment/card_transfer', 'token=' . $this->session->data['token'], 'SSL');
        ...
        if (isset($this->request->post['card_payment'])) {
            $this->data['payments'] = $this->request->post['card_payment'];
...
//print_r($this->data['payments']);die();
        ...
        $this->template = 'payment/card_transfer.tpl';
        $this->children = array(
            'common/header',
            'common/footer'
        );
    ...
    private function validate() {
        if (!$this->user->hasPermission('modify', 'payment/card_transfer')) {
        ...
        /*foreach ($languages as $language) {
            foreach($this->request->post['card_payment'] as $payment){
                if (!$payment['card_transfer_bank_' . $language['language_id']]) {
