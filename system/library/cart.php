			...
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$this->data[$type][$key] = array(
                        ...
                        'prevquantity' => isset($this->session->data['prevcart'][$key]) ? $this->session->data['prevcart'][$key] : 0
					);
				} else {
					$this->remove($key);
				}
  	...
  	public function update($key, $qty) {
    	if ((int)$qty && ((int)$qty > 0)) {
            ...
            if($this->session->data['cart'][$key] > 0){
                $this->session->data['prevcart'][$key] = $this->session->data['cart'][$key];
            }

            $this->session->data['cart'][$key] = 0;
	  		//$this->remove($key);
		}
